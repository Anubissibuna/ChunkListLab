...
namespace ChunkListLab {
	...
	public:
		...
		pointer allocate(size_type _Count) {
			auto ptr = static_cast<pointer>(::operator new(sizeof(value_type) * _Count));
			if (ptr)
				return ptr;
	...
	class Interface_Chunk {
	public:
		virtual ~Interface_Chunk() = default;
	...
	protected:
		int elem_index = 0;
		Interface_Chunk<ValueType>* list = nullptr;
	...
	public:
		...
		virtual int GetIndex() { return elem_index; };

		constexpr ChunkList_iterator() noexcept = default;

		ChunkList_iterator(Interface_Chunk<ValueType>* chunk, int index, ValueType* value) :
			list(chunk),
			elem_index(index),
			current_value(value)
		{};
		...
		friend bool operator<(const ChunkList_iterator<ValueType>& lhs,
			const ChunkList_iterator<ValueType>& rhs) {
			return rhs.elem_index - lhs.elem_index > 0;
		};
		friend bool operator<=(const ChunkList_iterator<ValueType>& lhs,
			const ChunkList_iterator<ValueType>& rhs) {
			return !(rhs.elem_index > lhs.elem_index);
		};
		friend bool operator>(const ChunkList_iterator<ValueType>& lhs,
			const ChunkList_iterator<ValueType>& rhs) {
			return rhs.elem_index < lhs.elem_index;
	...
	public:
		...
		ChunkList_const_iterator(Interface_Chunk<ValueType>* chunk, int index, ValueType* value) :
			ChunkList_iterator<ValueType>(chunk, index, value) {};

		ChunkList_const_iterator(const Interface_Chunk<ValueType>* chunk, int index, const ValueType* value) :
			ChunkList_iterator<ValueType>(const_cast<Interface_Chunk<ValueType>*>(chunk), index, const_cast<ValueType*>(value)) {};
	...
	class ChunkList : public Interface_Chunk<T> {
		class Chunk {
		public:
